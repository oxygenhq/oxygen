 ...
 */
function createProcess(processPath, startPath, args = [], env = null) {
    ...
    args = [processPath].concat(startPath+args);
 ...
 */
function executeWithInput(processPath, startPath, args = [], inputs = [], opts = {}) {
    if (!Array.isArray(inputs)) {
        opts = inputs;
        inputs = [];
    }

    const { env = null, timeout = 100, maxTimeout = 60 * 60 * 1000 } = opts;

    let scriptOutput = '';

    const childProcess = createProcess(processPath, startPath, args, env);
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        childProcess.stdout.on('data', data => {
            ...
            if (str.startsWith('Done!') || str.startsWith('Test failed:')) {
        ...
        childProcess.stderr.once('data', err => {
            ...
            const ignoreE = 'error: [Oxygen] Error initializing module "db": `unixodbc` binaries not installed';

            const errMessage = err.toString();

            if (errMessage.includes(ignoreE)) {
...
module.exports = {
    createProcess,
    create: (processPath, startPath) => {
        const fn = (...args) => executeWithInput(processPath, startPath, ...args);
