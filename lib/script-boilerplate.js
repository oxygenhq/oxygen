...
function handleStepResult(res, rs)
{
    ...
    if (res.CommandResult)
    {
        ...
        if (res.CommandResult.StatusText != null || res.CommandResult.ErrorMessage)
        {
			...
			step.failure._type = errorType = res.CommandResult.StatusText || null;
            step.failure._details = errorMessage = res.CommandResult.StatusData || null;
            var throwError = true;
			if (res.CommandResult.StatusText === 'VERIFICATION') 		// ignore verifyXXX commands failure
                    throwError = false;
			
			// check if exception message is presented
			...
			if (res.CommandResult.ErrorDetails) {
				errorStacktrace = res.CommandResult.ErrorDetails;
				if (step.failure._details == null)
					step.failure._details = res.CommandResult.ErrorDetails;	// always prefer less technical description in the details
			}
            if (throwError)
...
function moduleWrapper(name, module) {
	...
	Object.keys(module).forEach(function (methodName) {
		...
		wrapper[methodName] = function() {
			...
			var retval = module[methodName].apply(undefined, args);
			if (!methodName.indexOf('_') == 0 && methodName !== 'dispose') {	// do not report results when internal methods are called (started with '_')
				process.send({ event: 'result-update', module: name, method: methodName, retval: retval, args: args, results: resultStore, context: ctx, line: __line });	
			}
			return retval;
		}
	});
