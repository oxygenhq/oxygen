...
var AsyncError = require('../errors/AsyncError');
...
function loadModule(moduleName, moduleFileName, oxModulesDirPath, options) {
    var Module = require(path.join(oxModulesDirPath, moduleFileName));
...
function runScript(scriptContent, requireBaseFolder, name, path) {
    ...
    try {
        ...
        //        such exception don't get handled by fibers try/catch block
        ...
        process.on('uncaughtException', (e) => {
            ...
            reportResults(name, new AsyncError(e));
        });
...
function moduleWrapper(name, module, resultStore) {
    ...
    Object.keys(module).forEach(function (methodName) {
        ...
        wrapper[methodName] = function() {
            ...
            // throw if a command executed on unitialized module (except internal methods)
            if (!module._isInitialized() && publicMethod && methodName !== 'init') {
...
function requireWrapper(modulePath, baseFolder) {
    ...
    for (var modName in modules) {
        requiredModule[modName] = modules[modName];
    }
    ...
    if (e.stacktrace && typeof e.stacktrace === 'object') {
        ...
        for (call of calls) {
            if (call.indexOf(scriptPath) > -1) {
                var comp = call.split(':');
                if (comp.length == 3) {
                    retval.line = comp[1] - scriptContentLineOffset - 1;
