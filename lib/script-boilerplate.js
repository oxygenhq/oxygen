...
function init(options) {
	logger.debug('Starting initializing script boilerplate...');
	...
	var moduleFiles = globule.find("module-*.js", { srcBase: oxModulesPath });
	...
	try {
		for (var i = 0; i < moduleFiles.length; i++) {
			moduleName = moduleFiles[i];
			loadModule(moduleFiles[i], oxModulesPath, options);
		}
	} catch (e) {
		err = e;
...
function loadModule(moduleFile, oxModulesPath, options) {
	// extract module name
	...
	if (typeof moduleType === 'function') {
        ...
        if (mod.modType === 'dotnet' && initDotNetDispatcher || mod.modType === 'fiber' && !initDotNetDispatcher) {
            logger.debug('Loading module: ' + moduleName + ", type: " + mod.modType);
...
function runScript(path, content, name) {
    ...
    try {
        ...
        sandbox.doneCB = function(err, scriptContentLineOffset) { 
			if (err) {
				logger.debug('Error occured: ' + err.message);
			}
