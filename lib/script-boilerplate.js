...
function runScript(scriptContent, requireBaseFolder, name, path) {
    ...
    try {
        ...
        var unresolvedParams = new Set(scriptContent.match(/\${([A-Za-z0-9_]+)}/g));
        ...
        process.on('uncaughtException', (e) => {
            ...
            reportResults(name, new OxError(errHelper.errorCode.SCRIPT_ERROR, e.message, e));
        });
...
function reportResults(name, e) {
    ...
    if (err && err.line) {
        ...
        if (lastStep && lastStep.failure) {
            lastStep.failure._line = err.line;
        }
    }
