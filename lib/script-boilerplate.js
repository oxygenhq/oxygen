 ...
 */
// setup logger
const DEFAULT_LOGGER_ISSUER = 'runner';

var logger = {
    info: function(message, issuer = DEFAULT_LOGGER_ISSUER) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'INFO', msg: message, issuer });
        processSend({ time: ts, event: 'log-add', level: 'INFO', msg: message, issuer });
    },
    debug: function(message, issuer = DEFAULT_LOGGER_ISSUER) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'DEBUG', msg: message, issuer });
        processSend({ time: ts, event: 'log-add', level: 'DEBUG', msg: message, issuer });
    },
    error: function(message, err = null, issuer = DEFAULT_LOGGER_ISSUER) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'ERROR', msg: message, issuer });
        processSend({ time: ts, event: 'log-add', level: 'ERROR', msg: message, issuer, err: err });
    },
    warn: function(message, issuer = DEFAULT_LOGGER_ISSUER) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'WARN', msg: message, issuer });
        processSend({ time: ts, event: 'log-add', level: 'WARN', msg: message, issuer });
    }
};
    ...
    Object.keys(module).forEach(function (methodName) {
        ...
        wrapper[methodName] = function() {
            ...
            if (typeof(options.autoReport) === 'undefined' || options.autoReport == true || error) {
                handleStepResult(resultStore, module, name, methodName, args, startTime, endTime, retval, error);
            }
...
function handleStepResult(rs, module, moduleName, methodName, args, startTime, endTime, retval, err) {
    var step = new StepResult();
