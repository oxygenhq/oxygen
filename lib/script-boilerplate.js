...
var logger = {
    info: function(message) {
        ...
        resultStore.logs.push({ time: ts, level: 'INFO', msg: message });
        processSend({ time: ts, event: 'log-add', level: 'INFO', msg: message });
    },
    debug: function(message) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'DEBUG', msg: message });
        processSend({ time: ts, event: 'log-add', level: 'DEBUG', msg: message });
    },
    error: function(message, err) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'ERROR', msg: message });
        processSend({ time: ts, event: 'log-add', level: 'ERROR', msg: message, err: err });
    },
    warn: function(message) {
        const ts = getTimeStamp();
        resultStore.logs.push({ time: ts, level: 'WARN', msg: message });
        processSend({ time: ts, event: 'log-add', level: 'WARN', msg: message });
    }
};
