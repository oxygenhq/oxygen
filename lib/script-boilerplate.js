...
Object.defineProperty(global, '__line', {
    get: function () {
        ...
        return __stack[1].getLineNumber() - scriptContentLineOffset;
    }
});
...
function runScript(scriptContent, requireBaseFolder, name, path) {
    ...
    try {
        ...
        var script = new vm.Script(scriptWrapper, { filename: path, displayErrors: true });
        ...
        setTimeout(() => script.runInContext(context, {filename: path}), 1000);
    } catch (e) {
...
function requireWrapper(modulePath, baseFolder) {
    ...
    if (modulePath.indexOf('.') == -1 && modulePath.indexOf('/') == -1 && modulePath.indexOf('\\') == -1) {
        if (options.require && (options.require.allow || options.require.allowGlobal)) {
