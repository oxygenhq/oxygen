...
process.on('message', function (msg) {
    ...
    if (msg.type === 'init') {
        ...
        runScript(scriptContent, msg.scriptPath, msg.scriptName);
...
function runScript(scriptContent, scriptPath, name) {
    var vm = require('vm');
    ...
    try {
        ...
        if (allowRequire) {
            sandbox.require = function(modulePath) { return requireWrapper(modulePath, scriptPath); };
        }
...
function handleResult(rs, module, moduleName, methodName, args, duration, retval, err) {
    ...
    step._action = (typeof module._isAction === 'function' ? module._isAction.toString() : 'false');
...
function requireWrapper(modulePath, scriptPath) {
    var baseFolder = path.dirname(scriptPath);
    modulePath = oxutil.resolvePath(modulePath, baseFolder);
