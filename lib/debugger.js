...
const deleteValues = (arr, isEement = false, isBrowser = false) => {
    
    let result = arr.filter((item) => !blackList.includes(item.name));

    if(isEement){
        result = result.filter((item) => {
            if(item && item.value && item.value.className){
                return !['Function'].includes(item.value.className);
            } else {
                return true;
            }
        });

        result = result.filter((item) => !ElementBlackList.includes(item.name));
    }
    ...
    if(isBrowser){
        ...
        result = result.filter((item) => !BrowserBlackList.includes(item.name));
    }
...
class Debugger extends EventEmitter {
    ...
    async getPropertiesByObjectId(objectId, isEement = false, isBrowser = false){
        ...
        if(clone && clone && clone.result){
            clone = Object.assign({}, clone, {
                result: deleteValues(clone.result, isEement, isBrowser)
            });
        }
    ...
    async getProperties(objectId, isEement = false, isBrowser = false){
        const promises = [];

        const getPropertiesResult = await this.getPropertiesByObjectId(objectId, isEement, isBrowser);
            ...
            if(getPropertiesResult && getPropertiesResult.result && Array.isArray(getPropertiesResult.result)){
                ...
                const mapResult = getPropertiesResult.result.map(async (item) => {
                    if(item && item.value && item.value.objectId){
                        const getPropertiesInnerResult = await this.getProperties(item.value.objectId, item.value.className === "Element" || false, item.value.className === "Browser" || false);
        ...
        let scopeChainResult = await scopeChain.map(async (chain) => {
            if(chain && chain.object && chain.object.objectId){
                if(typeof chain.object.objectId === 'string' && chain.type !== 'global'){
                    ...
                    const getPropertiesResult = await this.getProperties(chain.object.objectId);
    ...
    async processCallFrames(callFrames){
        let callFramesMapResult = await callFrames.map(async(callFrame) => {
            if(callFrame && callFrame.scopeChain && Array.isArray(callFrame.scopeChain)){
                const processScopeResult = await this.processScope(callFrame.scopeChain);
                ...
                let breakpointsMapResult = this._breakpoints.map(async(bp) => {
                    if (bp &&
                        ...
                        if(e && e.callFrames && Array.isArray(e.callFrames) && e.callFrames.length > 0){
                            let callFramesMapResult = await this.processCallFrames(e.callFrames);
