...
const deleteValues = (arr) => {
    
    const result = arr.filter((item) => !blackList.includes(item.name));
...
class Debugger extends EventEmitter {
    ...
    async getPropertiesByObjectId(objectId){
        ...
        let saveInternalProperties = [];
        
        if(clone && clone.internalProperties){
            saveInternalProperties = clone.internalProperties;
        }

        if(clone && clone && clone.result){
            clone = Object.assign({}, clone, {
                result: deleteValues(clone.result)
            });
        }
    ...
    async getProperties(objectId, checkInner = false){
        const promises = [];

        const getPropertiesResult = await this.getPropertiesByObjectId(objectId);
        ...
        if(getPropertiesResult){
            if(getPropertiesResult && getPropertiesResult.result && Array.isArray(getPropertiesResult.result)){
                ...
                const mapResult = getPropertiesResult.result.map(async (item) => {
                    if(item && item.value && item.value.objectId && checkInner){
                        const getPropertiesInnerResult = await this.getProperties(item.value.objectId, true);
        ...
        let scopeChainResult = await scopeChain.map(async (chain) => {
            if(chain && chain.object && chain.object.objectId){
                if(typeof chain.object.objectId === 'string' && chain.type !== 'global'){
                    ...
                    const getPropertiesResult = await this.getProperties(chain.object.objectId, true);
