...
const deleteValues = (arr, isEement = false, isBrowser = false, depth) => {
...
class Debugger extends EventEmitter {
    ...
    async getPropertiesByObjectId(objectId, isEement = false, isBrowser = false, depth){
        ...
        if(clone && clone && clone.result){
            clone = Object.assign({}, clone, {
                result: deleteValues(clone.result, isEement, isBrowser, depth)
            });
        }
    ...
    async getProperties(objectId, isEement = false, isBrowser = false, depth){
        const promises = [];

        const getPropertiesResult = await this.getPropertiesByObjectId(objectId, isEement, isBrowser, depth);
            ...
            if(getPropertiesResult && getPropertiesResult.result && Array.isArray(getPropertiesResult.result)){
                ...
                const mapResult = getPropertiesResult.result.map(async (item) => {
                    if(item && item.value && item.value.objectId){
                        const getPropertiesInnerResult = await this.getProperties(item.value.objectId, item.value.className === "Element" || false, item.value.className === "Browser" || false, depth+1);
        ...
        let scopeChainResult = await scopeChain.map(async (chain) => {
            if(chain && chain.object && chain.object.objectId){
                if(typeof chain.object.objectId === 'string' && chain.type !== 'global'){
                    ...
                    const getPropertiesResult = await this.getProperties(chain.object.objectId, false, false, depth);
