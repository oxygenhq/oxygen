...
export default class Oxygen extends OxygenEvents {
    ...
    _loadServices() {
        ...
        const oxServicesDirPath = path.resolve(homeDir, './ox_services');
        ...
        for (var i = 0; i < serviceFiles.length; i++) {
            ...
            try {
                const service = this._loadService(serviceFilePath, this.opts);
                service.init();
    ...
    _commandWrapper(cmdName, cmdArgs, module, moduleName) {
        ...
        try {
            ...
            retval = this._wrapAsyncFunc(module[cmdName]).apply(module._this, cmdArgs);
    ...
    _wrapAsyncFunc(fn) {
        ...
        const executeFn = executeAsync
        const wrapInFiberContext = this._wrapInFiberContext.bind(this);
        return function (...args) {
            //console.log('calling async function')
            return executeFn.call(this, wrapInFiberContext(fn), 0, args);
        }
    }

    _wrapInFiberContext(code) {
    ...
    _callServicesOnModuleWillDispose(module) {
        for (let serviceName in this.services) {
            const service = this.services[serviceName];
            if (!service) {
                continue;
