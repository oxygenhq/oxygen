...
ReporterFileBase.prototype.createFolderStructureAndFilePath = function(fileExtension) {
    ...
    if (this.options.outputFolder && this.options.srcFile) {    // produce report at the specified path. will overwrite any existing reports.
        this.createFolderIfNotExists(this.options.outputFolder);
        var fileNameNoExt = oxutil.getFileNameWithoutExt(this.options.srcFile);
        return path.join(this.options.outputFolder, fileNameNoExt + fileExtension);
    } else if (this.options.srcFile) {                          // generate date-time folder structure for report files
        var baseDir = path.dirname(this.options.srcFile);
        var resultsMainFolderPath = path.join(baseDir, oxutil.getFileNameWithoutExt(this.options.srcFile));
