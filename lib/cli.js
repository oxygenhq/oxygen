...
if (fileExt === '.js') {
    tsReadyPromise = oxutil.generateTestCaseFromJSFile(srcFile, startup.parameters.file, startup.parameters.mode);    
} else if (fileExt === '.xml') {
    tsReadyPromise = oxutil.generateTestSuiteFromXmlFile(srcFile);
} else if (fileExt === '.json') {
    tsReadyPromise = oxutil.generateTestSuiteFromJsonFile(srcFile, startup.parameters.file, startup.parameters.mode, startup);
} else {
    console.error('Unsupported file format: ' + fileExt);
    process.exit(1);
}
// check if the returned result is a promise
if (typeof tsReadyPromise.then === 'function') {
	...
	prepareAndStartTheTest(tsReadyPromise);
}
