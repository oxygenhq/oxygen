...
const LoadLauncher = require('./launcher-load');
...
General options:
  -d, --delay=SECONDS        Delay between each command in seconds. 
      --rf={html|pdf|xml|excel|junit|csv}  Reports file format. Default is html.
  ...
  -p, --param=FILE           Parameters file. If not specified an attempt will
      ...
      --load={true|false}    Run Oxygen in load testing mode. Default is false.
      --parallel             Amount of concurrent (parallel) tests.
      --duration             Maximum duration (in seconds) of the test if more iterations left to run.
      --rampup               Ramp up time (in seconds) to get to the maximum parallel tests amount.
...
var startup = {
    ...
    maxDuration : argv.duration ? parseInt(argv.duration) : null,
    parallel : argv.parallel ? parseInt(argv.parallel) : null,
    rampup : argv.rampup ? parseInt(argv.rampup) : 0,
    debugPort: argv.dbgport || null,
    loadTestingMode: argv.load ? argv.load === 'true' : false,
...
// adjust report format to use CSV reporter if we are using load launcher (e.g. load testing mode)
if (startup.loadTestingMode) {
    startup.reporter.format = 'csv';
}

// TODO: temporary - remove!
/*const results = require(path.join(path.dirname(srcFilePath), 'result1.json'));
...
.catch(function(err) {
    console.error('Failed to start the test.', err);
    process.exit(1);
});
...
function prepareAndStartTheTest(ts) {
    ...
    if (ts.options && ts.options.reopenSession && ts.options.reopenSession !== startup.reopenSession) {
        startup.reopenSession = ts.options.reopenSession;
    }
    console.log('startup.reopenSession', startup.reopenSession)
    ...
    var capsArr = ts.capabilities || null;
    ...
    if (startup.ext && startup.ext.browserStack && capsArr) {
        _.each(capsArr, function(caps) {
            addBrowserStackCapabilities(caps, startup);
        });
    }
    // start launcher - in case we are in load testing mode, use LoadLauncher class
    const launcher = startup.loadTestingMode ? new LoadLauncher(ts, startup) : new Launcher(ts, startup);
