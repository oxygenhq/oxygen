...
General options:
  ...
  -p, --param=FILE           Parameters file. If not specified an attempt will
                             be made to load paramteres from a file named same
      ...
      --pm={seq|random|all}  Order in which to read the parameters - sequential, 
                             ...
                             In all mode -i option will be ignored and all 
                             available parameters will be read sequentialy.
...
Mob mode options:
      --host=HOSTNAME        Appium server's hostname. Default is localhost.
      --port=PORT            Appium server's port. Default is 4723.`);
    process.exit(1);
}
