...
var Runner = module.exports.Runner = function () {
	...
	var _caps = null;	// desired capabilities that are passed to each module
	var _options = null;
    // promises
    ...
    module.init = function (options) { /*args, mode, debugPort*/
        ...
        childProc = fork(path.join(__dirname, 'script-boilerplate.js'), null, forkExecArgv);
        // hook all events of the child process
        hookChildProcEvents(childProc);
        // initialize debugger
        // apply the breakpoints and request continue
        if (debugMode) {
            initializeDebugger(options.debugPort);
        }
    ...
    module.run = function (testsuite, envVars, caps) {
		...
		_caps = caps || null;	// assign caps for later use
        ts = testsuite;
    ...
    function hookChildProcEvents() {
        ...
        childProc.on('message', function (msg) {
            //logger.debug('Message from script-boilerplate: ' + msg);
    ...
    function processResultsAndRunTheNextIteration(msg) {
        ...
        else if (tsit > ts.iterationCount) {
            finalizeTest(msg.error);
        } else {
            runTestCase();
        }
    ...
    function finalizeTest(error) {
        ...
        resetGlobalVariables();
        /*if (fatalError) {
			console.log('fatal error detected: ' + testResults);
			_whenFinished.reject({
				results: testResults,
				error: fatalError
			});
		}
