...
var Runner = module.exports.Runner = function () {
    ...
    module.init = function (args, debugPort) {
		...
		args.push('--useFiber=' + config.get('useFiber'));
	...
	function hookChildProcEvents() {
		...
		childProc.on('exit', function (code, signal) {
			...
			if (code && code != 0) {
				//logger.debug('script-boilerplate process exited with code: ' + code + ', signal: ' + signal);
				// if the test is running or is being disposed and the child process has died, then finish the test or disposal with fatal error (child process not suppose to die with code > 0)
				var promise, error;
				if (_isRunning || _isDisposing) {
					promise = _isRunning ? _whenFinished : _isDisposed;
    ...
    function finalizeTest()
    {
        ...
        tr.$.startTime = startTime.toISOString();
        tr.$.endTime = endTime.toISOString();
