...
module.exports.Runner = function () {
    ...
    module.init = function (options) {
        ...
        if (options.debugPort) {            // when debugging from VS Code
            debugPort = options.debugPort;
            forkOpt = Object.assign(forkOpt, { execArgv: ['--inspect-brk=' + debugPort] });
        } else if (options.debugPortIde) {  // when debugging from Oxygen IDE
            debugMode = true;
            debugPort = options.debugPortIde;
            forkOpt = Object.assign(forkOpt, { execArgv: ['--inspect-brk=' + debugPort] });
        }
        ...
        // if we are in debug mode from Oxygen IDE, initialize debugger and only then start modules 'init'
        if (debugMode) {
            initializeDebuggerDelayed(debugPort, childProc, options);
        } else {
    ...
    module.updateBreakpoints = function(breakpoints, filePath) {
        try {
            var tc;
            ...
            if (debugMode && dbg && tc) {
                ...
                const tsBreakpoints = dbg.getBreakpoints(filePath);

                // add new breakpoints
                for (var userSetBp of breakpoints) {
                    if (!tsBreakpoints.includes(userSetBp)) {
                        promises.push(dbg.setBreakpoint(filePath, userSetBp));
                    }
                }
                ...
                for (var actualCurrentBp of tsBreakpoints) {
                    if (!breakpoints.includes(actualCurrentBp)) {
                        promises.push(dbg.removeBreakpointByValue(filePath, actualCurrentBp));
                    }
                }
    ...
    function hookChildProcEvents() {
        ...
        childProc.on('exit', function (code, signal) {
            var dbgMsg = 'script-boilerplate process exited with code: ' + code + ', signal: ' + signal;
            if (dbg) {
                dbg.close();
            }
            if (code && code !== 0) {
                ...
                if (_isRunning) {
                    let error;
                    if (code == 134 /*out of memory*/) {
                        error = new Error('Out of memory. Make sure that you don\'t have any memory leaks in the test script.');
                    }

                    processCriticalFailureResults({
                        results: null,
                        error: error
                    });

                    // resolve _whenDisposed since it won't be resolved due to 'modules-disposed' not happening because the child proc did not exit properly
                    _whenDisposed.resolve(null);
                    resetGlobalVariables();
                } else if (_isDisposing) {
                    _whenDisposed.reject(_childProcLastError || new Error(dbgMsg));
                    resetGlobalVariables();
                }
            } else {
                logger.debug(dbgMsg);
        ...
        childProc.on('message', function (msg) {
            if (msg.event && msg.event === 'log-add') {
                ...
                if (msg.src === DEFAULT_ISSUER) {
                    ee.emit('log-add', msg.level, msg.msg, msg.time);
                }
                ...
                ee.emit('line-update', msg.stack, msg.time);
    ...
    const DEBUGGER_INIT_ATTEMPTS = 10;

    function initializeDebuggerDelayed(debugPort, childProc, options, initAttempt) {
        // delay debugger initialization, as debugger port might not be open yet right after the process fork
        if (!initAttempt) {
            initAttempt = 0;
        }
        setTimeout(function() {
            initializeDebugger(debugPort, childProc, options).catch(e => {
                if (++initAttempt >= DEBUGGER_INIT_ATTEMPTS) {
                    _isInitializing = false;
                    _whenInitialized.reject(e);
                    logger.error('Error initializing debugger.', e);
                } else {
                    logger.info(`Error initializing debugger. Retrying ${initAttempt}/${DEBUGGER_INIT_ATTEMPTS}...`);
                    initializeDebuggerDelayed(debugPort, childProc, options, initAttempt);
                }
            });
        }, 500);
    }

    async function initializeDebugger(debugPort, childProc, options) {
        ...
        dbg.on('break', function(breakpoint, variables) {
            // when debugging from Oxygen IDE, breakpoint.hitBreakpoints will have always only one element
            // since we cannot set multiple breakpoints on same location
            
            // console.log('breakpoint', JSON.stringify(breakpoint, null, 2));
            // console.log('variables', JSON.stringify(variables, null, 2));

            if (breakpoint.callFrames.length > 0) {
                let topFrame = breakpoint.callFrames[0];

                // console.log('topFrame', topFrame);

                let breakpointData =  {
                    fileName: topFrame.url,
                    lineNumber: parseInt(topFrame.location.lineNumber),
                    columnNumber: parseInt(topFrame.location.columnNumber)
                };
                ...
                ee.emit('breakpoint', breakpointData);
            }
        });
        // connect to Chrome debugger
        await dbg.connect(debugPort);
    }
    ...
    function runTestCase() {
        ...
        if (debugMode && dbg && tc.breakpoints) {
            ...
            for (let i=0; i< tc.breakpoints.length; i++) {
                ...
                let line = tc.path === file ? module.getScriptContentLineOffset + tc.breakpoints[i].line : tc.breakpoints[i].line - 1;
    ...
    function processResultsAndRunTheNextIteration(msg) {
        ...
        // workaround for case & suite iterations not being defined due to CB backend bug
        if (!tc.iterationCount) {
            tc.iterationCount = 1;
        }
        if (!ts.iterationCount) {
            ts.iterationCount = 1;
        }
    ...
    function processCriticalFailureResults(msg) {
        var tc = ts.testcases[tcindex];
    ...
    function GenerateFailureObject(error) {
        ...
        failure._type = error.type ? error.type : 'UNKNOWN_ERROR';
