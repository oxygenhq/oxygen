        ...
        if (debugMode) {
            ...
            setTimeout(function() {
                initializeDebugger(debugPort, childProc, options).catch(e => {
                    ...
                    logger.error('Can not initialize Debugger', e);
                }); 
            }, 1000);
        } else {
    ...
    module.updateBreakpoints = function(breakpoints, filePath) {
        try {
            ...
            if (
                ...
                tc && 
                tc.breakpoints && 
                Array.isArray(tc.breakpoints)
            ) {
                let promises = [];

                const tsBreakpoints = tc.breakpoints.filter(br => { 
                    return br.file === filePath; 
                }).map(br => { return br.line; });

                const tsBreakpointsCopy = [...tsBreakpoints];
                const breakpointsCopy = [...breakpoints];
                const deleteBreakpoints = [];
                const addBreakpoints = [];

                tsBreakpointsCopy.map(tsBr => {
                    if(breakpointsCopy.includes(tsBr)){
                        // element is in old and new Breakpoints
                        // ignore
                    } else {
                        // Breakpoint removed, push to delete array
                        deleteBreakpoints.push(tsBr);
                    }
                });

                breakpointsCopy.map(br => {
                    if(tsBreakpointsCopy.includes(br)){
                        // element is in old and new Breakpoints
                        // ignore
                    } else {
                        // new breakpoint added, push to add array
                        addBreakpoints.push(br);
                    }
                });
            
                deleteBreakpoints.map(breakpoint => {
                    ...
                    let line = tc.path === filePath ? module.getScriptContentLineOffset + breakpoint : breakpoint - 1;
                    promises.push(dbg.removeBreakpointByValue(filePath, line));
                });
                ...
                addBreakpoints.map(breakpoint => {
                    ...
                    // for secondary files just reduce by 1 since BP indices are 0-based
                    let line = tc.path === filePath ? module.getScriptContentLineOffset + breakpoint : breakpoint - 1;
                    promises.push(dbg.setBreakpoint(filePath, line));
                });
