...
module.exports.Runner = function () {
    ...
    module.init = function (options) {
        ...
        if (options.debugPort) {
            debugMode = true;
            forkOpt = Object.assign(forkOpt, { execArgv: ['--inspect-brk=' + options.debugPort] });
            ...
            debugMode = true;
            forkOpt = Object.assign(forkOpt, { execArgv: ['--inspect-brk=' + options.debugPortIde] });
        }
    ...
    function initializeDebugger(debugPort, childProc, options) {
        ...
        dbg.on('ready', function(err) {
            dbg.continue(); // resume the first breakpoint
        ...
        dbg.on('break', function(breakpoint) {
            if (breakpoint.hitBreakpoints.length == 0) {
                dbg.continue();
                return;
            }
    ...
    function runTestCase() {
        ...
        if (debugMode && dbg && tc.breakpoints) {
            for (let i=0; i< tc.breakpoints.length; i++) {
                ...
                let line = tc.breakpoints[i].line - 1;
                ...
                logger.debug('Oxygen set initial bps: ' + (line));
