...
module.exports.Runner = function () {
    ...
    var tcStartTime = null; // current test case start time (of the first iteration)
        ...
        if (debugMode) {
            ...
            setTimeout(function() {
                initializeDebugger(debugPort, childProc, options).catch(e => {
                    ...
                    _whenInitialized.reject(e.message);
                }); 
            }, 1000);
        } else {
    ...
    function hookChildProcEvents() {
        ...
        childProc.on('message', function (msg) {
            if (msg.event && msg.event === 'log-add') {
                ...
                ee.emit('ui-log-add', msg.level, msg.msg, msg.time);
    ...
    function runTestCase() {
        if (tcit == 1) {
            tcStartTime = moment.utc().unix();
        }
        ee.emit('iteration-start', tcit);
        var tc = ts.testcases[tcindex];
        var params = {};
        console.log(`Starting iteration ${tcit} of suite "${ts.name}" and test case "${tc.name}"...`);
        ...
        // send run-script message to script boilerplate
        var msg = {
            type: 'run-script',
            scriptName: tc.name,
            scriptPath: tc.path,
    ...
    function processResultsAndRunTheNextIteration(msg) {
        ...
        tcit++;
        // calculate test case duration in seconds (from the first iteration onwards)
        var duration = tcStartTime ? moment.utc().unix() - tcStartTime : 0;
        if (tcit > tc.iterationCount || (tc.maxDuration && duration >= tc.maxDuration)) {
            tcindex++;
            tcit = 1;
        }
