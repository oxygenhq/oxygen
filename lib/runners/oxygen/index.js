...
import ReporterXLSX from '../../reporters/excel-reporter.js';
...
export default class OxygenRunner extends EventEmitter {
    ...
    async _runTest() {
        ...
        if (error) {
            result.failure = errorHelper.getFailureFromError(error); // this._generateFailureObject(error);
    ...
    async _runCase(suite, caze, suiteIteration, caseIteration) {
        ...
        try {
            ...
            if (error) {
                caseResult.failure = error; //errorHelper.getFailureFromError(error); //this._generateFailureObject(error);
    ...
    async _initializeDebugger() {
        ...
        this._dbg.on('ready', function(err) {
            ...
            this._dbg.continue(); 
            this._whenDebuggerReady.resolve();
        });
        this._dbg.on('error', function(err) {
            ...
            if (!this._whenDebuggerReady.isResolved) {
                this._whenDebuggerReady.reject(err);
            }
...
export { ParameterManager, ReporterXLSX };