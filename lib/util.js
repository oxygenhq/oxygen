...
var self = module.exports = {
    ...
    locatePageObjectFile: function(filePath) {
        const baseFolder = path.dirname(filePath);
        var fileName = self.getFileNameWithoutExt(filePath);
        // check if <script/suite name>.po.json file exists
        var directPoFile = path.join(path.dirname(filePath), fileName + '.po.json');
        // check if po.json file exists
        var globalPoFile = path.join(path.dirname(filePath), 'po.json');

        if (fs.existsSync(directPoFile)) {
            return directPoFile;
        } else if (fs.existsSync(globalPoFile)) {
            return globalPoFile;
        }
        return null;
    }
